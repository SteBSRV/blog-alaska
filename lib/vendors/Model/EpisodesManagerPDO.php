...
class EpisodesManagerPDO extends EpisodesManager
{
  ...
  {
    $requete = $this->dao->prepare('INSERT INTO episodes SET author = :author, title = :title, content = :content, addDate = NOW(), modDate = NOW(), state = :state');
    ...
    $requete->bindValue(':state', $episodes->getState());
  ...
  {
    return $this->dao->query('SELECT COUNT(*) FROM comments WHERE episode_id = ' . (int) $id)->fetchColumn();
  }
  ...
  {
    $sql = 'SELECT id, title, author, content, addDate, modDate, state FROM episodes WHERE state = 1 ORDER BY id DESC';
  ...
  public function getListAdmin($debut = -1, $limite = -1)
  {
  ...
  {
    $requete = $this->dao->prepare('UPDATE episodes SET author = :author, title = :title, content = :content, modDate = NOW(), state = :state WHERE id = :id');
